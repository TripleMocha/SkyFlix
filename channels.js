...
let hlsPlayer = null; // HLS.js fallback
...
async function playChannel(channel) {
  ...
  try {
    ...
    if (type === "HLS") {
      // Use native HLS or HLS.js fallback
...
// Example: generate channel cards
function loadChannels(channels) {
  channelContainer.innerHTML = "";
  channels.forEach((ch) => {
    const card = document.createElement("div");
    card.className = "channel-card";
    card.innerHTML = `<img src="${ch.logo}" alt="${ch.name}" /><p>${ch.name}</p>`;
    card.onclick = () => {
...
const channels = [
  ...
  { name: "PBA RUSH", genre: "Sports", logo: "../assets/img/channel logo/pba_rush.jpg", stream: "https://qp-pldt-live-grp-01-prod.akamaized.net/out/u/cg_pbarush_hd1.mpd", drm: "NzZkYzI5ZGQ4N2EyNDRhZWFiOWU4YjdjNWRhMWU1ZjM6OTViMmYyZmZkNGUxNDA3MzYyMDUwNjIxM2I2MmFjODI=" },
   {
   name: 'UAAP',
     genre: "Sports",
